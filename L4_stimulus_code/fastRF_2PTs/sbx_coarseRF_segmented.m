function [RF_center,dFoF_perstim,std_perstim,dFoF_traces,otplanes]=sbx_coarseRF_segmented(fn,fr)
%fast (~5-10 minutes) analysis to determine 2 photon RF
%3/9/20- sanity check with segement and motion reg
...
%if nargin<1
    ...
    fr=15.49;
    plane_i = 2;
    load -mat 'ES16_001_000_ot_001_rigid.signals'
%elseif nargin<2
%    fr=15.49;
%    
%end
...
%%
 fp_trial = [];
test= dir([fn '.sbx']);
info.max_idx= test.bytes/info.sz(1)/info.sz(2)/1/2;
[on,off,otplanes, filel] = sbx_trialinfo(info);

if info.volscan~=0 %if volume scan
    on=otplanes{plane_i}(1:2:end);
    off=otplanes{plane_i}(2:2:end);
    fp_trial=vertcat(fp_trial,off-on);
else
    fp_trial=vertcat(fp_trial, off-on);
end
%% get dF/F per segmented ROIs
dFoF_traces=nan(ceil(mean(off-on)),numel(on),size(sig,2));

for i = 1:numel(on)
    nsamps_stretch = ceil(mean(off-on));
    nsamps_orig = numel(on(i):off(i));
    trial_traces = interp1(sig(on(i):off(i),:),...
        [1:(nsamps_orig-1)/(nsamps_stretch-1):nsamps_orig]')';    %normalize roi_F trace to baseline 2 s preceding trace.
    
    roi_b=mean(trial_traces(:,1:floor(ceil(mean(off-on))*stim_time(1)/6)),2);
    roi_dFoF=((trial_traces-roi_b)./roi_b);
    
    dFoF_traces(1:size(roi_dFoF,2),i,:)= reshape(roi_dFoF',size(roi_dFoF,2),1,[]); %eventually sort these so that you can average for same trial types.
    dFoF(i,:)=mean(roi_dFoF(:,ceil(ceil(mean(off-on))/6*stim_time(1)+1):floor(ceil(mean(off-on))/6*(stim_time(1)+stim_time(2)))),2)'; %store average dF/F during stimulus (row = trial, column = ROI)
    
    if mod(i,50)==0
        disp(['working on trial # ' int2str(i)]);
    end
end
...
for a=1:numel(apos)
    ...
    dFoF_perstim(a,:)=mean(dFoF(apos_in(:,a),:));
    std_perstim(a,:)=std(dFoF(apos_in(:,a),:))...
        /sqrt(length(dFoF(apos_in(:,a),:)));
    
    apos_in_r(:,a)= ismember(trial_nums,[apos(a),1],'rows');
    dFoF_perstim_r(a,:)=mean(dFoF(apos_in_r(:,a),:));
    std_perstim_r(a,:)=std(dFoF(apos_in_r(:,a),:))...
        /sqrt(length(dFoF(apos_in_r(:,a),:)));
    ntrials(a,1)=numel(find(apos_in(:,a)==1));
    ntrials(a,2)=numel(find(apos_in_r(:,a)==1));
    
    figure(2); subplot(2,16,a); hold on;
    slicecol = { [0.25 0.25 0.25], [1 0.5 0.5]};
    
    plot(squeeze(mean(dFoF_traces(:,apos_in(:,a),:),2)),'Color','c','LineWidth',0.1);
    
    %         plot(dFoF_traces(:,apos_in(:,a),3),'m','LineWidth',0.1);
    plot(mean(mean(dFoF_traces(:,apos_in(:,a),:),3),2),'k','LineWidth',2);
    xlim([0 fr*6]);
    ylim([-0.1 0.5]);
    ycords=ylim;
    ...
    figure(2); subplot(2,16,a+16); hold on;
    
    plot(squeeze(mean(dFoF_traces(:,apos_in_r(:,a),:),2)),'Color','c','LineWidth',0.1);
    
    plot(mean(mean(dFoF_traces(:,apos_in_r(:,a),:),3),2),'k','LineWidth',2);
    xlim([0 fr*6]);
    ylim([-0.1 0.50]);
    area(fr*stim_time(1):fr*(stim_time(1)+stim_time(2)), ycords(2)*ones(1, numel(fr*stim_time(1):fr*(stim_time(1)+stim_time(2)))),...
        ycords(1),'FaceColor','k','FaceAlpha',0.1,'LineStyle','none')
...
figure; clf; hold on;
plot(dFoF_perstim);
plot(mean(dFoF_perstim,2),'k','LineWidth',3);
errorbar(mean(dFoF_perstim,2),std(dFoF_perstim,[],2)./sqrt(size(dFoF_perstim,2)),'ko');
%% TO DO:
...
for i=1:nslices
   ...
   for a=1:numel(apos)
        apos_in(:,a)= ismember(trial_nums,[apos(a),0],'rows');
        dFoF_perstim(a,:)=mean(dFoF(apos_in(:,a),i,m));
